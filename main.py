...
TOKEN = "5693601947:AAH0aqwKMOmL33dom2AtRWvHgd3XGB-awVY"  # Токен Вильсона
...
def welcome(message):
    ...
    if str(uid) not in users:
        ...
        if not users[str(uid)]['status']:
            users[str(uid)]['status'] = True
            bot.send_message(message.chat.id,
...
def task_initiation(message):
    ...
    if attempt_local == 3:
        bot.send_message(message.from_user.id, f'{data["#" + task_number]["description"]}\nУ вас есть {users[str(uid)]["remaining_attempts"]["#" + task_number]} попытки',
                     reply_markup=keyboard_restart())
    else:
        bot.send_message(message.from_user.id,
                         f'{data["#" + task_number]["description"]}\nУ вас осталось {users[str(uid)]["remaining_attempts"]["#" + task_number]} попытки',
                         reply_markup=keyboard_restart())
                        ...
                        if data[actual_task]["type_out"] == "NO":
                            if users[str(uid)]['score'][actual_task] <= users[str(uid)]['best_score'][actual_task]:
                                ...
                                bot.send_message(uid,
                                                 ...
                                                 f'Чтобы начать снова нажмите: Продолжить подготовку.', reply_markup=keyboard_restart())
